...
class Agent {
     ...
     */
    constructor(reasoner, maxSteps = 8, cozeOrchestrator) {
    ...
    async run(userQuery) {
        ...
        if (this.cozeOrchestrator && this.shouldUseCozeOrchestration(userQuery)) {
            try {
                this.history.push({
                    role: 'agent',
                    content: '检测到复杂任务，使用Coze Studio服务编排处理...',
                });
        ...
        for (let step = 0; step < this.maxSteps; step++) {
            ...
            if (parsed.finalAnswer) {
                this.history.push({
                    role: 'agent',
                    content: `Final Answer: ${parsed.finalAnswer}`,
                });
            ...
            if (this.cozeOrchestrator &&
                this.shouldUseCozeOrchestrationAfterTool(toolName, observation)) {
                try {
                    this.history.push({
                        role: 'agent',
                    ...
                    const orchestrationResult = await this.cozeOrchestrator.executeTask(`基于工具执行结果处理原始任务: ${userQuery}\n\n工具执行结果: ${observation}`, undefined, this.history);
                    this.history.push({
                        role: 'agent',
                        content: `Coze Studio服务编排结果: ${orchestrationResult}`,
                    });
     ...
     */
    shouldUseCozeOrchestration(userQuery) {
        // 简单示例：检查查询中是否包含特定关键词
     ...
     * @returns 是否需要使用Coze Studio服务编排
     */
    shouldUseCozeOrchestrationAfterTool(toolName, observation) {
