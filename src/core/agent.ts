...
export class Agent {
  ...
  async run(
    userQuery: string
  ): Promise<{ final: string; history: HistoryItem[] }> {
    // 为新查询重置历史记录
    this.history = [];
    this.textHistory = [];
    ...
    if (this.cozeOrchestrator && this.shouldUseCozeOrchestration(userQuery)) {
      try {
        ...
        const orchestrationResult =
          await this.cozeOrchestrator.executeTask(userQuery);
        ...
        console.error(
          'Coze Studio服务编排执行失败，回退到标准ReAct流程:',
          error
        );
    ...
    for (let step = 0; step < this.maxSteps; step++) {
      ...
      try {
        // 使用提供的输入执行工具
        observation = await toolFn(
          parsed.actionInput?.query ?? parsed.actionInput ?? ''
        );
      } catch (e: any) {
        // 处理工具执行错误
      ...
      if (
        ...
        try {
          ...
          console.error(
            'Coze Studio服务编排执行失败，继续标准ReAct流程:',
            error
          );
